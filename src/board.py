...
class Board:
    ...
    def __init__(self,dim :int = None, board : np.ndarray = None):
        ...
        if board.shape[0]!=board.shape[1]:
            warnings.warn("init warning");print(f'Board array should be square, the input board has the shape ({board.shape})')
        
        if dim!=board.shape[0]:
            warnings.warn("init warning");print(f'dim and the board dimensions do not match. The dim would be set as the shape of the board, {dim}!={board.shape[0]}')
    ...
    def goal_board(self):
        ...
        """
        board = np.r_[:self.dim][None,:]+[[1+i] for i in range(0,self._max_no,self.dim)]
        board[-1,-1] = 0
        return board
    ...
    def get_new_states(self):
        ...
        states = {}
        candidates = {"u":(zero_i-1,zero_j), "d":(zero_i+1,zero_j), "l":(zero_i,zero_j-1), "r":(zero_i,zero_j+1)}
        for key  in candidates.keys():
            i,j = candidates[key]
            if i>=0 and j>=0 and i<self.dim and j<self.dim:
                states[key] = Board.from_arr(self.slide(self.board.copy(),zero_i,zero_j,i,j))
        return states
    ...
    def get_random_board(cls, dim:int ,max_iter: int = 1000):
        ...
        i=0
        while i<max_iter:
            board = np.random.choice(dim*dim,dim*dim, replace=False).reshape(dim,dim)
    ...
    def from_arr(cls, arr: np.ndarray):
        ...
        """
        assert arr.shape[0]==arr.shape[1], f"The input array should be square. the provided array has shape ({arr.shape})"
        n = arr.shape[0]
